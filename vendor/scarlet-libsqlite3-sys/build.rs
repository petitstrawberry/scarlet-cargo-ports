...
mod build_bundled {
    ...
    pub fn main(out_dir: &str, out_path: &Path) {
        ...
        // Scarlet currently has no dlopen C API. Cargo never loads SQLite
        // extensions; omitting that interface avoids linking a false stub.
        if env::var("TARGET").is_ok_and(|target| target.ends_with("-scarlet")) {
            cfg.flag("-DSQLITE_OMIT_LOAD_EXTENSION");
        } else {
