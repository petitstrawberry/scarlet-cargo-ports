...
pub fn bytes2path(b: &[u8]) -> &Path {
    // Scarlet std uses the byte-preserving OsStr representation, so every
    // byte boundary from libgit2 is an encoded OsStr boundary.
    Path::new(unsafe { OsStr::from_encoded_bytes_unchecked(b) })
}
#[cfg(windows)]
pub fn bytes2path(b: &[u8]) -> &Path {
    use std::str;
...
impl IntoCString for OsString {
    ...
    #[cfg(target_os = "scarlet")]
    fn into_c_string(self) -> Result<CString, Error> {
        Ok(CString::new(self.as_encoded_bytes())?)
    }
}
