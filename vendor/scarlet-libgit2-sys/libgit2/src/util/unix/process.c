...
/* Native process creation is exposed through Rust std on Scarlet. libgit2's
 * Unix implementation depends on fork-time signal masks and a global environ
 * pointer, neither of which has a C ABI yet. Reject external filters clearly. */
struct git_process { int unused; };

static int unsupported_process(void)
{
	git_error_set(GIT_ERROR_OS, "external git processes are unavailable on Scarlet");
	return -1;
}
...
{
	(void)args; (void)args_len; (void)env; (void)env_len; (void)opts;
	if (out) *out = NULL;
	return unsupported_process();
}

int git_process_new_from_cmdline(git_process **out, const char *cmdline,
	const char **env, size_t env_len, git_process_options *opts)
{
	(void)cmdline; (void)env; (void)env_len; (void)opts;
	if (out) *out = NULL;
	return unsupported_process();
}

int git_process_start(git_process *process)
{ (void)process; return unsupported_process(); }
int git_process_id(p_pid_t *out, git_process *process)
{ (void)out; (void)process; return unsupported_process(); }
ssize_t git_process_read(git_process *process, void *buf, size_t count)
{ (void)process; (void)buf; (void)count; return unsupported_process(); }
...
{ (void)result; (void)process; return unsupported_process(); }
...
{ (void)process; return unsupported_process(); }
int git_process_result_msg(git_str *msg, git_process_result *result)
{ (void)msg; (void)result; return unsupported_process(); }
void git_process_free(git_process *process) { (void)process; }
