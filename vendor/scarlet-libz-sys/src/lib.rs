...
#[cfg(all(
    not(zng),
    feature = "libc",
    not(target_os = "scarlet"),
    not(all(target_family = "wasm", target_os = "unknown"))
))]
pub type z_off_t = libc::off_t;

// Scarlet's C sys/types.h defines off_t as a signed 64-bit integer. The
// upstream libc crate does not yet expose that alias for this target.
...
#[cfg(all(zng, not(target_os = "scarlet"), not(all(windows, not(target_env = "gnu")))))]
pub type z_off_t = libc::off_t;
