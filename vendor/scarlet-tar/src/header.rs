...
#[cfg(all(any(unix, windows, target_os = "scarlet"), not(target_arch = "wasm32")))]
...
impl Header {
    ...
    fn fill_platform_from(&mut self, meta: &fs::Metadata, mode: HeaderMode) {
        self.set_uid(0);
        self.set_gid(0);
        self.set_mtime(match mode {
            HeaderMode::Complete => meta
                .modified()
                .ok()
                .and_then(|time| time.duration_since(std::time::UNIX_EPOCH).ok())
                .map_or(0, |duration| duration.as_secs()),
            HeaderMode::Deterministic => DETERMINISTIC_TIMESTAMP,
        });
        // Scarlet std currently exposes read-only but not the execute bit.
        // Report only the permissions that can be represented accurately.
        let writable = !meta.permissions().readonly();
        self.set_mode(match (meta.is_dir(), writable) {
            (true, true) => 0o755,
            (true, false) => 0o555,
            (false, true) => 0o644,
            (false, false) => 0o444,
        ...
        self.set_entry_type(if kind.is_dir() {
            EntryType::dir()
...
#[cfg(target_os = "scarlet")]
fn ends_with_slash(p: &Path) -> bool {
    p.to_str().is_some_and(|path| path.as_bytes().ends_with(b"/"))
}
...
#[cfg(target_os = "scarlet")]
/// Scarlet paths are UTF-8 and use `/` as the only separator.
pub fn path2bytes(p: &Path) -> io::Result<Cow<[u8]>> {
    p.to_str()
...
#[cfg(any(target_arch = "wasm32", target_os = "scarlet"))]
pub fn bytes2path(bytes: Cow<[u8]>) -> io::Result<Cow<Path>> {
    Ok(match bytes {
        Cow::Borrowed(bytes) => {
