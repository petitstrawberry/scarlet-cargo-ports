...
// Scarlet currently has no process UID or file owner identity. Existing
// paths must receive reduced trust rather than being treated as current-user
// owned on the strength of a fabricated UID.
#[cfg(target_os = "scarlet")]
mod impl_ {
    pub fn is_path_owned_by_current_user(path: &std::path::Path) -> std::io::Result<bool> {
        std::fs::symlink_metadata(path)?;
        Ok(false)
    }
}

#[cfg(all(not(windows), not(target_os = "wasi"), not(target_os = "scarlet")))]
mod impl_ {
    use std::path::Path;
