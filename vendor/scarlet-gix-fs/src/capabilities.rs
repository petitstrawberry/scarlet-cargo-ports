...
impl Default for Capabilities {
    fn default() -> Self {
        Capabilities {
            precompose_unicode: false,
            ignore_case: false,
            // Scarlet's public std API does not expose executable permissions
            // or symlink creation yet. Probing can only enable these when the
            // corresponding operations are actually available.
            executable_bit: false,
            symlink: false,
        }
    }
